...
const removePageHeader = (value) =>
  value
    .replace(
      /\s+(?:\d{2}\s+o\s+)?EXAME\s+(?:DO|DE)\s+ORDEM\s+UNIFICADO\s+Tipo Branca\s+[–-]\s+Página\s+\d+\s*$/i,
      "",
    )
    .trim();
...
function extractAnswerKey(text) {
  ...
  const typeTwoStart = lines.findIndex(
    (line, index) => index > typeOneStart && /PROVA TIPO 2/i.test(line),
  );
  const typeOneLines = lines.slice(
    typeOneStart,
    typeTwoStart > -1 ? typeTwoStart : undefined,
  );
  ...
  for (let index = 0; index < typeOneLines.length - 1; index += 1) {
    ...
    if (
      ...
      !letters.every((letter) => /^[A-E]$/i.test(letter))
    )
      continue;
    if (
      numbers.length !== letters.length ||
      numbers.some((number) => number < 1 || number > 80)
    )
      continue;
    numbers.forEach((number, answerIndex) =>
      answers.set(number, letters[answerIndex].toUpperCase()),
    );
  }
  return answers;
}
...
function parseQuestions(text, examNumber, answerKey) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    if (/^(?:[1-9]|[1-7]\d|80)$/.test(lines[index])) {
      ...
      const isPageHeader = followingLines.some((line) =>
        /^(?:Tipo Branca\b|EXAME\b|o$|SUA PROVA|TEMPO|INFORMAÇÕES GERAIS|NÃO SERÁ PERMITIDO)$/i.test(
          line,
        ),
      );
      if (number >= 1 && number <= 80 && !isPageHeader)
        markers.push({ index, number });
    }
  }

  const firstQuestion = markers.findIndex(
    (marker, index) => marker.number === 1 && markers[index + 1]?.number === 2,
  );
  if (firstQuestion === -1) return [];

  const questions = [];
  for (
    let markerIndex = firstQuestion;
    markerIndex < markers.length;
    markerIndex += 1
  ) {
    const marker = markers[markerIndex];
    if (marker.number > 80 || marker.number !== markerIndex - firstQuestion + 1)
      break;
    ...
    const block = lines
      ...
      .filter(
        (line) =>
          !/^(?:\d{2}|o|EXAME DE ORDEM|EXAME DO ORDEM|Tipo Branca\b|SUA PROVA|TEMPO|INFORMAÇÕES GERAIS|NÃO SERÁ PERMITIDO)$/i.test(
            line,
          ),
      );
    const optionStarts = [];
    ...
    const statement = removePageHeader(
      block.slice(0, optionStarts[0].lineIndex).join(" ").replace(/\s+/g, " "),
    );
    const options = optionStarts.map((option, optionIndex) => {
      const end = optionStarts[optionIndex + 1]?.lineIndex ?? block.length;
      return removePageHeader(
        [option.text, ...block.slice(option.lineIndex + 1, end)]
          .join(" ")
          .replace(/\s+/g, " "),
      );
    });
...
async function main() {
  ...
  for (const exam of ["45", "46", "47"]) {
    ...
    questions.push(
      ...parseQuestions(
        examDocument.text,
        exam,
        extractAnswerKey(keyDocument.text),
      ),
    );
  }
  ...
  for (const file of files) {
    ...
    const structured = {
      ...
      exam,
      documentType: baseName.endsWith("-prova")
        ? "prova"
        : baseName.endsWith("-gabarito")
          ? "gabarito"
          : "pesquisa",
    };
    if (structured.documentType === "prova") {
      const answerDocument = documents.get(`${exam}-gabarito.pdf`);
      structured.questions = parseQuestions(
        document.text,
        exam,
        answerDocument ? extractAnswerKey(answerDocument.text) : new Map(),
      );
    } else if (structured.documentType === "gabarito") {
      structured.answers = Object.fromEntries(extractAnswerKey(document.text));
    } else {
      structured.sections = document.text
        .split(/\n{2,}/)
        .map((section) => section.trim())
        .filter(Boolean);
    }
    await writeFile(
      join(extractedDir, `${baseName}.json`),
