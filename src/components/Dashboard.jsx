export default function Dashboard({ attempts }) {
  const total = attempts.reduce((n, a) => n + a.total, 0),
    correct = attempts.reduce((n, a) => n + a.correct, 0);
  const averageDuration = attempts.length
    ? Math.round(
        attempts.reduce((n, a) => n + (a.duration || 0), 0) / attempts.length,
      )
    : 0;
  ...
  const clearHistory = () => {
    if (
      !window.confirm(
        "Limpar todo o histórico de tentativas deste dispositivo?",
      )
    )
      return;
    ...
    <section>
      <div className="page-heading">
        <div>
          ...
          <p>Acompanhe seu ritmo e evolua a cada tentativa.</p>
        </div>
      </div>
      <div className="stats">
        <div>
      ...
      <div className="panel">
        ...
        {attempts.length ? (
              ...
              .map((a, i) => (
                <div className="attempt" key={i}>
                  ...
                  <b>
                    {a.correct}/{a.total}
                    <br />
                    <small>{a.errors ?? a.total - a.correct} erros</small>
                  </b>
                </div>
              ))}
          </div>
          ...
          <p className="muted">
            Nenhuma tentativa registrada ainda. Faça um quiz para ver seu
            histórico aqui.
          </p>
        )}
      </div>
