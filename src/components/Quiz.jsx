...
export default function Quiz({
  ...
  if (!questions.length)
    return (
      <section className="empty-state">
        <span>✦</span>
        <h2>Banco de questões em preparação</h2>
        <p>
          Adicione as questões extraídas dos PDFs em{" "}
          <code>src/data/questions.json</code> para começar o quiz.
        </p>
      </section>
    );
  const answer = (option) => {
  ...
  const restart = () => {
    if (
      !window.confirm("Reiniciar o simulado? O progresso atual será perdido.")
    )
      return;
    const now = Date.now();
  ...
  const next = () => {
    ...
    if (index + 1 < questions.length) {
      ...
      saveAttempt({
        ...
        correct: nextCorrect,
        errors: questions.length - nextCorrect,
        duration,
  ...
  const selectedLetter =
    selected !== null ? String.fromCharCode(65 + selected) : null;
  ...
  if (done)
    return (
      <section className="empty-state">
        ...
        <p>Sua tentativa foi salva no desempenho.</p>
        <button
          className="primary"
          onClick={() => {
    ...
    <section className={`quiz-card ${paused ? "quiz-paused" : ""}`}>
      <div className="quiz-toolbar">
        ...
        <div className="quiz-controls">
          <button className="secondary" onClick={onTogglePause}>
      ...
      <div className="options">
        {q.options.map((option, i) => (
          <button
            key={option}
            disabled={paused}
            className={`option ${selected !== null ? (i === q.answer ? "correct" : i === selected ? "wrong" : "") : ""}`}
            onClick={() => answer(i)}
          >
      ...
      {selected !== null && (
        ...
        >
          ...
          {!isCorrect && (
            <span>
              {" "}
              A alternativa correta é <b>{correctLetter}</b>.
            </span>
          )}
          {hasRealExplanation && (
            <>
      ...
      {paused && (
        <div className="pause-notice" role="status">
          Simulado pausado. Seu tempo está congelado.
        </div>
      )}
    </section>
  );
